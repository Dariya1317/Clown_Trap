...
public final class AudioManager implements Disposable {
    ...
    private static final float HEARTBEAT_INTERVAL = 0.85f;
    ...
    private AudioManager() {
        ...
        // Heartbeat (short one-shot sound, fired on interval)
        try {
            heartbeat = Gdx.audio.newSound(Gdx.files.internal("audio/heartbeat.ogg"));
        } catch (Exception e) {
            heartbeat = null;
        }
     ...
     * Fires heartbeat sound on interval at <= 5 s; restores ambient when inactive.
     */
    public void updateHeartbeat(boolean active, float dt) {
        if (heartbeat == null) return;
        if (active) {
            if (!heartbeatActive) {
                heartbeatActive = true;
                heartbeatTimer  = 0f;          // play immediately on first frame
                if (current != null) current.setVolume(0.0f);
            }
            heartbeatTimer -= dt;
    ...
    public void dispose() {
        ...
        ambients.clear();
        if (heartbeat     != null) { heartbeat.stop(); heartbeat.dispose(); heartbeat = null; }
